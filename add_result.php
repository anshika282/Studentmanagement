...
$mark=$_POST['marks'];
 ...
 $stmt ="SELECT subjname,sbid FROM subject  WHERE c_id='$class' order by subjname";
 $stmtr1=mysqli_query($con,$stmt);
 $sid1=array();
 while($row=mysqli_fetch_array($stmtr1))
 {

array_push($sid1,$row['sbid']);
   } 
  
for($i=0;$i<count($mark);$i++){
    $mar=$mark[$i];
  $sid=$sid1[$i];
$resultins="INSERT INTO  result(stuid,classid,subid,marks) VALUES('$studentid','$class','$sid','$mar')";
$resultinsert =mysqli_query($con,$resultins);
...
{   ?>
<option value="<?php echo htmlentities($result['cid']); ?>"><?php echo htmlentities($result['cstd']); ?>(<?php echo htmlentities($result['section']); ?>)</option>
<?php }} ?>
 </select>
                                                        </div>
                                                    </div>
